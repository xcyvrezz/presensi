...
class CheckIn extends Component
{
    ...
    /**
     * Process check-in (called from JavaScript after NFC scan + GPS capture)
     */
    public function processCheckIn()
    {
        ...
        try {
            ...
            if ($result['success']) {
                ...
                // Dispatch success event to JavaScript for sound/vibration
                $this->dispatch('checkInSuccess', data: $result['data']);
            } else {
                $this->dispatch('checkInFailed', message: $result['message']);
            }

        } catch (\Illuminate\Validation\ValidationException $e) {
            $this->isSuccess = false;
            $this->message = $e->validator->errors()->first();
            $this->dispatch('checkInFailed', message: $this->message);
        } catch (\Exception $e) {
            $this->isSuccess = false;
